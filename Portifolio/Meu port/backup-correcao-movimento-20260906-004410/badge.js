...
(() => {
 ...
 function tick(time){frame=null;const dt=Math.min((time-last)/1000||.016,.032);last=time;
   ...
   if(visible&&!document.hidden&&!motion.matches&&!resting)frame=requestAnimationFrame(tick);
 }
 ...
 function swing(direction=1){if(motion.matches){x=direction*20;angle=direction*-5;paint();return;}vx=clamp(vx+direction*360,-650,650);vy-=85;spin-=direction*60;run();}
 ...
 body.addEventListener('pointermove',e=>{
   ...
   if(!drag){if(e.pointerType==='mouse'&&brush&&!motion.matches){const dx=e.clientX-brush.x;if(Math.abs(dx)>1&&now-brush.time<100){vx=clamp(vx+dx*2,-260,260);spin=clamp(spin-dx*.34,-100,100);run();}brush={x:e.clientX,time:now};}return;}
   ...
   if(motion.matches){x=drag.tx;y=drag.ty;angle=-x*.1;paint();}else run();
 });
 function release(){if(!drag)return;if(!motion.matches&&performance.now()-drag.lastT<100){vx=clamp(vx+drag.handVX*.42,-700,700);vy=clamp(vy+drag.handVY*.25,-500,500);spin=clamp(spin-drag.handVX*.086,-150,150);}clearDrag();if(motion.matches){vx=vy=spin=0;paint();}else run();}
