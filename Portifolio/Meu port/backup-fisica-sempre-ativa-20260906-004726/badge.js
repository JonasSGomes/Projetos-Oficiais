...
(() => {
 ...
 const physicsToggle=document.getElementById('badgePhysics');
 let physics=!motion.matches,physicsChoice=null;
 ...
 function swing(direction=1){if(physicsChoice===null)enablePhysics();if(!physics){x=direction*20;angle=direction*-5;paint();return;}vx=clamp(vx+direction*360,-650,650);vy-=85;spin-=direction*60;run();}
 ...
 body.addEventListener('pointerdown',e=>{if(e.button!==0||!e.isPrimary||drag)return;if(physicsChoice===null)enablePhysics();const r=stage.getBoundingClientRect();brush=null;drag={id:e.pointerId,ox:e.clientX-r.left-width/2-x,oy:e.clientY-r.top-y,tx:x,ty:y,handVX:0,handVY:0,lastX:e.clientX,lastY:e.clientY,lastT:performance.now()};body.setPointerCapture(e.pointerId);body.classList.add('dragging');body.focus({preventScroll:true});run();});
 ...
 document.addEventListener('visibilitychange',()=>{if(document.hidden){release();stop();}else run();});addEventListener('blur',release);motion.addEventListener('change',()=>{if(physicsChoice===null){physics=!motion.matches;syncPhysics();center();}});
 function syncPhysics(){
   physicsToggle.textContent=physics?'Física: ligada':'Ativar física';
   physicsToggle.setAttribute('aria-pressed',String(physics));
 }
 function enablePhysics(){physics=true;syncPhysics();}
 physicsToggle.addEventListener('click',()=>{
   physicsChoice=!physics;physics=physicsChoice;syncPhysics();
   if(physics){swing();status.textContent='Física ativada para o crachá.';}
   else{center();status.textContent='Física desativada. O crachá pode ser movido sem balanço automático.';}
 });
 syncPhysics();measure();if(!physics)center();else{y=15;run();}
})();
